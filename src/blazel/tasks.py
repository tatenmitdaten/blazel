...
Data = Iterator[tuple] | list[tuple]
...
ExtractLoadSchema = BaseSchema[ExtractLoadWarehouseType, ExtractLoadSchemaType, ExtractLoadTableType]
ExtractLoadWarehouse = BaseWarehouse[ExtractLoadWarehouseType, ExtractLoadSchemaType, ExtractLoadTableType]
...
class TaskOptions(BaseOptions, Serializable):
    ...
    fail_on_error: Literal['true', 'false'] = 'false'
...
@dataclass
class TableTask(
    BaseTask[ExtractLoadWarehouseType],
    Generic[ExtractLoadWarehouseType, ExtractLoadTableType]
):
...
class ErrorTask(TableTask):
    ...
    def __call__(self, warehouse: ExtractLoadWarehouseType, _=None) -> dict | None:
        ...
        raise TestError(f'{self.task_type} Test Error')
...
class LoadTask(TableTask):
    ...
    def __call__(self, warehouse: ExtractLoadWarehouseType, _=None) -> dict | None:
        ...
        if self.truncate is not None:
            table.meta.truncate = self.truncate
...
class ExtractLoadJob(Serializable):
    ...
    def from_table(cls, table: ExtractLoadTable, task_options: TaskOptions | None = None) -> 'ExtractLoadJob':
        ...
        if options.start is None and table.meta.look_back_days:
            options.batches = table.meta.look_back_days if table.meta.timestamp_key else 1
            tzinfo = zoneinfo.ZoneInfo(table.meta.timezone)
            end_datetime = datetime.datetime.now(tz=tzinfo)
            start_datetime = end_datetime - datetime.timedelta(days=table.meta.look_back_days - 1)
            options.start = start_datetime.strftime('%Y-%m-%d')
            options.end = end_datetime.strftime('%Y-%m-%d')
        options.batches = max(options.batches, table.meta.batches)
...
class Schedule(Serializable):
    ...
    @property
    def as_dict(self) -> dict:
        return super().as_dict if self.schedule else {'schedule': []}
...
class ScheduleTask(BaseTask[ExtractLoadWarehouseType]):
    ...
    def __post_init__(self):
        if self.database_name is None:
            self.database_name = BaseWarehouse.get_database_name()
...
class TimeRange(Generic[ExtractLoadTableType]):
    ...
    max_end_str: ClassVar[str] = datetime.datetime.now().replace(hour=23, minute=59, second=59).strftime(
    ...
    def from_task(cls, task: ExtractTask | ScheduleTask, table: ExtractLoadTableType) -> 'TimeRange':
        ...
        if start is None and table.meta.timestamp_field:
